...
func main() {
	...
	logTest1Deal := log.New(os.Stdout, "Test 1 Deals", log.LstdFlags|log.Lshortfile)
	test1Handler := handlers.NewTest1Handler(logTest1Deal)

	router.Methods(http.MethodPost).Subrouter().HandleFunc("/mangtas/test1", test1Handler.GetTop10UsedWords)
